...
export default function Questions({ mcq, timeLeft }: QuestionsProps) {
  ...
  const givenAnswer = (ans: string) => {
    ...
    addedScore(
      mcq?.options[mcq.answer as keyof typeof mcq.options] === ans ? 10 : 0
    );
  ...
  return (
    <div>
      ...
      <div className="text-white flex flex-wrap justify-center text-2xl font-semibold italic pt-10">
        <span className="animate-fade-down animate-once animate-duration-1000 animate-delay-100">
          {mcq?.question}
        </span>
      </div>
      ...
      <div className="flex items-center flex-col gap-4 justify-center py-[2%]">
        {/* Green Neon Border */}
        <div
          onClick={() => givenAnswer(mcq.options.a)}
          className={`relative w-[50%] cursor-pointer h-16 mb-4 animate-fade-left ${
            answer === mcq.options.a ? "bg-teal-600" : ""
          }`}
        >
          <div className="absolute inset-0 border-4 border-green-500 rounded-lg opacity-80 flex justify-center items-center text-white text-xl">
            {mcq?.options?.a}
          </div>
        ...
        {/* Yellow Neon Border */}
        <div
          onClick={() => givenAnswer(mcq.options.b)}
          className={`relative w-[50%] cursor-pointer h-16 mb-4 animate-fade-right ${
            answer === mcq.options.b ? "bg-teal-600" : ""
          }`}
        >
          <div className="absolute inset-0 border-4 border-yellow-500 rounded-lg opacity-80 flex justify-center items-center text-white text-xl">
            {mcq?.options?.b}
          </div>
        ...
        {/* Blue Neon Border */}
        <div
          onClick={() => givenAnswer(mcq.options.c)}
          className={`relative w-[50%] cursor-pointer h-16 mb-4 animate-fade-left ${
            answer === mcq.options.c ? "bg-teal-600" : ""
          }`}
        >
          <div className="absolute inset-0 border-4 border-blue-500 rounded-lg opacity-80 flex justify-center items-center text-white text-xl">
            {mcq?.options?.c}
          </div>
        ...
        {/* Red Neon Border */}
        <div
          onClick={() => givenAnswer(mcq.options.d)}
          className={`relative w-[50%] cursor-pointer h-16 mb-4 animate-fade-right ${
            answer === mcq.options.d ? "bg-teal-600" : ""
          }`}
        >
          <div className="absolute inset-0 border-4 border-red-500 rounded-lg opacity-80 flex justify-center items-center text-white text-xl">
            {mcq?.options?.d}
          </div>
